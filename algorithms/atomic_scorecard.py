...
Contributors:
  Thomas Husmann — framework, shell-lining insight, bronze exchange hypothesis
  Grok (xAI)    — d-block edge diagnosis, leg-pinning concept
  Claude (Anthropic) — mode selector, quantum depth, s-valve formalization

═══════════════════════════════════════════════════════════════════════════════
THE FORMULA — v5: The S-Electron Valve
═══════════════════════════════════════════════════════════════════════════════

Three modes for all atoms, plus three sub-modes for d-block:

  s/p-block:  ratio = BASE + n_p × g₁ × φ^(-(per-1))         [ADDITIVE]
  noble gas:  ratio = √(1 + (θ × BOS)²)                      [PYTHAGOREAN]
  d-block:    depends on the s-electron valve:

    LEAK MODE    — boundary d (n_d≤4 or n_d≥9) WITH s-electron:
                   ratio = 1 + 1/φ⁴
                   The s-electron opens the bronze gate (σ₄ boundary).
                   Energy leaks through to σ₅ (quantum dark sector).
                   The outer wall = 1 (covalent) + leakage fraction.
                   1/φ⁴ = 0.1459 = 1 - r_c (complement of crossover onset).

    REFLECT MODE — d¹⁰ with NO s-electron (only Pd in periods 4-5):
                   ratio = BASE + dark_gold / φ⁴
                   Bronze gate is SHUT. Energy reflects off gold layer.
                   The gold dark fraction adds to BASE at the leak rate.
                   Pd observed: 1.453. Predicted: 1.451. Error: 0.2%.

    STANDARD     — mid-series d⁵-d⁸:
                   ratio = √(1 + (θ × BOS)²)
                   θ = 1 - (n_d/10) × dark_gold
                   Active bonding shields the exchange. Normal Pythagorean.

The s-electron is a VALVE:
  Open (s present at boundary): energy flows OUT through bronze → compressed
  Shut (no s at d¹⁰):          energy reflects IN off gold → expanded

Real electron configurations used for 8 anomalous elements (NIST measured):
  Cr:[Ar]3d⁵4s¹  Cu:[Ar]3d¹⁰4s¹  Nb:[Kr]4d⁴5s¹  Mo:[Kr]4d⁵5s¹
  Ru:[Kr]4d⁷5s¹  Rh:[Kr]4d⁸5s¹   Pd:[Kr]4d¹⁰     Ag:[Kr]4d¹⁰5s¹

═══════════════════════════════════════════════════════════════════════════════
CONSTANTS (all from φ² = φ + 1, D = 233)
═══════════════════════════════════════════════════════════════════════════════

  BASE = σ₄/σ_shell = 1.408382      g₁ = 0.324325 (sub-gap fraction)
  BOS = bronze_σ₃/σ_shell = 0.992   dark_gold = 0.290
  LEAK = 1/φ⁴ = 0.1459              (transmission through Cantor barrier)
  1 + LEAK = 1.1459                  (leak mode ratio)
  BASE + dark_gold×LEAK = 1.4507     (reflect mode ratio)

═══════════════════════════════════════════════════════════════════════════════
RESULTS: mean 7.6%, 38/54 within 10%, 53/54 within 20% (98%)
         d-block: mean 6.0%, 17/20 within 10% — BEST block
         Only B exceeds 20% (-29.6%, the F(9) boundary problem)
═══════════════════════════════════════════════════════════════════════════════

  v1 additive:     mean 10.4%, <10%=31, <20%=45 (83%)
  v3 Hybrid C:     mean  9.5%, <10%=31, <20%=51 (94%)
  v4 edge lining:  mean  8.3%, <10%=36, <20%=53 (98%)
  v5 s-valve:      mean  7.6%, <10%=38, <20%=53 (98%)

OPEN: B/C (F(9) boundary), Co (mid-series HCP), halogens Cl/Br/I (+16-18%)
...
LEAK = 1/PHI**4  # 0.14590 — the Cantor barrier transmission fraction
...
# Real electron configurations deviating from Madelung (NIST measured)
REAL_CONFIG = {
...
def build_spectrum():
    ...
    return R_SHELL, R_OUTER, R_INNER, R_MATTER, sg[0]/s3w

R_SHELL, R_OUTER, R_INNER, R_MATTER, G1 = build_spectrum()
BASE = R_OUTER/R_SHELL; BOS = BRONZE_S3/R_SHELL

# Derived mode constants
...
def predict_ratio(Z):
    ...
    if block == 'd':
        ...
        return BASE+n_p*G1*PHI**(-(per-1)), per, n_p, n_d, n_s, block, "additive"
...
def print_header():
    print("="*80)
    print("  ATOMIC SCORECARD v5 — The S-Electron Valve")
    print("  Husmann Decomposition: φ²=φ+1, D=233, zero free parameters")
    print("="*80)
    print(f"\n  BASE={BASE:.6f}  g₁={G1:.6f}  BOS={BOS:.6f}  dark_gold={DARK_GOLD}")
    print(f"  LEAK=1/φ⁴={LEAK:.6f}  RATIO_LEAK={RATIO_LEAK:.4f}  RATIO_REFLECT={RATIO_REFLECT:.4f}")
    print()
...
def print_cat1(results):
    print("-"*85)
    print("  CATEGORY 1: v5 Ratio Formula (s-valve + real configs)")
    print("-"*85)
    print(f"\n  {'Z':>3} {'Sym':>3} {'Blk':>3} {'nd':>2} {'ns':>2} {'Mode':>9} {'Pred':>7} {'Obs':>7} {'Err':>7} {'QD':>3}")
    for r in results:
        m="+" if abs(r['err'])<10 else ("~" if abs(r['err'])<20 else "X")
        star='★' if r['real'] else ' '
        print(f"  {r['Z']:3d} {r['sym']:>3} {r['block']:>3} {r['n_d']:>2} {r['n_s']:>2}{star}"
              f" {r['mode']:>9} {r['pred']:7.3f} {r['obs']:7.3f} {r['err']:+7.1f}% {r['qd']:3d} {m}")
...
def print_element(Z):
    ...
    print(f"\n  {sym} (Z={Z})  Period {per}, n_d={n_d}, n_s={n_s}, Block: {block} {real}")
    print(f"  Mode: {mode}")
    if mode=='leak':
        print(f"  ratio = 1 + 1/φ⁴ = 1 + {LEAK:.4f} = {RATIO_LEAK:.4f}")
        print(f"  (s-electron opens bronze gate → energy leaks to σ₅)")
    elif mode=='reflect':
        print(f"  ratio = BASE + dark_gold/φ⁴ = {BASE:.4f} + {DARK_GOLD}×{LEAK:.4f} = {RATIO_REFLECT:.4f}")
        print(f"  (no s-electron → gate shut → energy reflects off gold layer)")
    elif mode=='standard':
        theta=1-(n_d/10)*DARK_GOLD
        print(f"  θ = 1 - ({n_d}/10)×{DARK_GOLD} = {theta:.4f}")
        print(f"  ratio = √(1 + ({theta:.4f}×{BOS:.4f})²) = {rp:.4f}")
    elif mode=='pythagorean':
        theta=1+n_p*(G1/BOS)*PHI**(-(per-1))
        print(f"  θ = {theta:.4f}")
        print(f"  ratio = √(1 + ({theta:.4f}×{BOS:.4f})²) = {rp:.4f}")
    else:
        print(f"  ratio = {BASE:.4f} + {n_p}×{G1:.4f}×φ^(-{per-1}) = {rp:.4f}")
    if Z in RADII:
...
def main():
    ...
    c2=[{'name':'H-H bond','pred':predict_h_bond(),'obs':74.14,'note':'σ₄/σ_sh×a₀'},
        {'name':'H vdW','pred':predict_h_vdw(),'obs':120.0,'note':'σ₄/σ_sh×φ×a₀'},
        {'name':'S_max','pred':predict_smax(),'obs':1.408377,'note':'σ₄/σ_sh'},
        {'name':'He vdW','pred':predict_he_vdw(),'obs':140.0,'note':'σ₄/σ_sh×φ×(1+β)×a₀'}]
    n,n10,n5=print_simple(2,"DIRECT — H and He",c2)
    totals.append(("Direct H/He",n,n10,n5,"S_max: 0.00021%"))
    c3=[{'name':'α⁻¹','pred':predict_alpha_inv(),'obs':137.036,'note':'N×W'},
        {'name':'a₀','pred':predict_bohr(),'obs':52.918,'note':'ℏ/(mₑcα)'},
        {'name':'Ry','pred':predict_rydberg(),'obs':13.606,'note':'mₑc²α²/2'},
        {'name':'r_p','pred':predict_rp(),'obs':0.8414,'note':'λ_C×φ^(3-β)'}]
    n,n10,n5=print_simple(3,"SPECTRAL — α=1/(N×W)",c3)
    totals.append(("Spectral",n,n10,n5,"r_p: 0.14%"))
    c4=[{'name':'5+8=13','pred':13,'obs':13,'note':'Discriminant'},
        {'name':'σ₄ Pythag','pred':math.sqrt(R_SHELL**2+BRONZE_S3**2),'obs':R_OUTER,'note':'√(sh²+br²)'},
        {'name':'σ₂=gold_σ₃','pred':GOLD_S3,'obs':0.235,'note':'Inner wall'},
        {'name':'BASE Pythag','pred':predict_base_pyth(),'obs':BASE,'note':'√(1+BOS²)'}]
    ...
    c7=[{'name':'Ω_b','pred':predict_omega_b(),'obs':0.0493,'note':'W⁴'},
        {'name':'He E₂/E₁','pred':predict_he_ratio(),'obs':2.213,'note':'√5'},
    ...
    print(f"  {'Category':<35s} {'Tests':>5} {'<10%':>7} {'<5%':>7}  Best");print(f"  {'-'*70}")
    gn=g10=g5=0
    ...
    print();print("-"*80);print("  FLAGSHIPS");print("-"*80)
    for nm,er in [("S_max","0.00021%"),("t_as","0.005%"),("Cantor Pythag","0.012%"),("r_p","0.14%"),
        ("Cs","0.2%"),("Pd REFLECT","0.2%"),("α⁻¹","0.22%"),("H vdW","0.5%"),("Y LEAK","0.6%"),
        ("Zn LEAK","0.6%"),("Kr","1.2%"),("Bonds","1.7%")]:
        print(f"    {er:>12s}  {nm}")
    print();print("-"*80);print("  OPEN PROBLEMS");print("-"*80)
    for nm,er,nt in [("B,C","-30%,-19%","F(9) boundary"),("Co","-16%","mid-series HCP"),
        ("Cl,Br,I","+16-18%","late p-block overshoot"),("He abs","3.9%","Z_eff")]:
        print(f"    {nm:<15s} [{er:>10s}]  {nt}")
    print();print("-"*80);print("  THE S-ELECTRON VALVE");print("-"*80)
    print(f"""
  The s-electron is a VALVE controlling the σ₄ (bronze) boundary:

    OPEN (s present):  energy leaks through bronze to σ₅ → atom COMPRESSES
                       ratio = 1 + 1/φ⁴ = {RATIO_LEAK:.4f}
                       Examples: Cu(d¹⁰s¹)=1.061, Y(d¹s²)=1.153, Zn(d¹⁰s²)=1.139

    SHUT (no s):       energy reflects off gold layer → atom EXPANDS  
                       ratio = BASE + dark_gold/φ⁴ = {RATIO_REFLECT:.4f}
                       Example: Pd(d¹⁰)=1.453 — predicted 1.451, error 0.2%

    BONDING (d⁵-d⁸):  d-electrons actively bond, shielding the exchange
                       ratio = √(1 + (θ×BOS)²), standard Pythagorean

  1/φ⁴ = {LEAK:.6f} = 1 - r_c = the Cantor barrier transmission fraction.
  Same constant as: crossover onset, W⁴ baryon mechanism, existence condition.
""")
