...
**March 2026 | v2: Pythagorean triangle, corrected nesting**
...
**3. The natural length scale** l₀ = 9.3 nm appears as a G/hBN moiré period at twist angle θ = 1.17°, where 38 graphene unit cells ≈ 37 hBN unit cells form a commensurate approximant (0.31% match). At the magic angle in a G/hBN device, the moiré period is 9.76 nm — within 5% of l₀.
...
At golden flux (α = 1/φ), the five-band Cantor partition carries Chern numbers +2, −1, +1, −2. The outer pair (+2, −2) annihilates via topological pair annihilation, collapsing five bands to three — the 5→3 mechanism supported by Liu, Fulga & Asbóth (2020). The first three metallic mean discriminants Δ\_n = n² + 4 are consecutive Fibonacci numbers (5, 8, 13), forming a Pythagorean triple (√5)² + (√8)² = (√13)² that closes at exactly three spatial dimensions. The Dirac dispersion E² = p²c² + m²c⁴ maps onto 13 = 5 + 8, with silver (innermost, 83% dark) as mass and gold (middle, 29% dark) as momentum. 36 supporting references span Hofstadter spectroscopy, moiré physics, Floquet topology, and metallic mean quasicrystals.
...
### III.D. The Natural Length Scale l₀ = 9.3 nm

The AAH Hamiltonian at the critical point defines a natural coherence length $l_0 = \hbar c / (2J)$ where $J = 10.578$~eV is the hopping parameter. This gives $l_0 = 9.327$~nm. This scale appears in the graphene/hBN system as a moiré period at twist angle:
...
The natural length scale $l_0$ coincides with where **38 graphene unit cells ≈ 37 hBN unit cells**, forming a near-commensurate approximant. In Zeckendorf representation: 38 = F(9) + F(4) + F(2) = 34 + 3 + 1, and 37 = F(9) + F(4) = 34 + 3.
...
If exact, this would mean λ\_max = φ × l₀, connecting the n = 60 graphene shell directly to the natural length scale through the golden ratio.
...
This matches to **0.03%**. The magnetic length at one flux quantum per l₀² plaquette is l₀/√(2π) — a consequence of the flux quantum definition and the geometric relation between circular (magnetic) and square (lattice) areas.
...
This is **proven algebra**:

$$\varphi^2(1 - 1/\varphi^4) = \varphi^2 - 1/\varphi^2 = \frac{\varphi^4 - 1}{\varphi^2} = \frac{(\varphi^2 - 1)(\varphi^2 + 1)}{\varphi^2} = \frac{\varphi(\varphi^2 + 1)}{\varphi^2} = \frac{\varphi^2 + 1}{\varphi} = \frac{\varphi + 2}{\varphi} = 1 + \frac{2}{\varphi} = 2\varphi - 1 = \sqrt{5} \quad \square$$
...
| Commensurability at l₀ | 38 × a\_g = 37 × a\_hBN = l₀ | Natural length scale is a graphene/hBN commensurate resonance |

---

## X. Connection to Previous Results

### X.A. N-SmA Universality
...
### X.B. The Cantor Crossover Operator
...
### X.C. GABA as Measurement Operator
...
## XII. The Discriminant Pythagorean Triangle
...
|---|---|---|---|---|
| 1 | Golden: φ | **5** = F(5) | ✓ | √5 = 2.236 |
| 2 | Silver: 1+√2 | **8** = F(6) | ✓ | √8 = 2.828 |
| 3 | Bronze: (3+√13)/2 | **13** = F(7) | ✓ | √13 = 3.606 |
| 4 | n=4 | 20 ≠ F(8) = 21 | ✗ | — |

The recurrence F(5) + F(6) = F(7) — which is **5 + 8 = 13** — holds for the first three discriminants. At n = 4, the chain breaks: 8 + 13 = 21 ≠ 20.

**Uniqueness proof:** The recurrence Δ_{n-1} + Δ_n = Δ_{n+1} requires (n-1)² + 4 + n² + 4 = (n+1)² + 4, which gives (n-2)² = 0, so n = 2. The silver mean is the UNIQUE link. Exactly one consecutive triple works: {5, 8, 13}. ∎

### The Pythagorean Relation

The relation 5 + 8 = 13 is simultaneously a Fibonacci recurrence AND a Pythagorean relation:

$$(√5)² + (√8)² = (√13)²$$

The three discriminant square roots form a right triangle with legs √5 (gold) and √8 (silver) and hypotenuse √13 (bronze):
...
The angle at the gold vertex: θ_g = arctan(√8/√5) = 51.67°, with cos θ_g = √5/√13 = 0.6202 ≈ 1/φ.

### Concentric Nesting (Corrected)

The three metallic means nest concentrically by their observer band width (σ₃, the central band of the AAH spectrum at V = 2J):

| Layer | Mean | σ₃ width | Dark fraction | Role |
|---|---|---|---|---|
...
The narrowest observer band = most confined = innermost. Silver nests inside gold, gold inside bronze. Confirmed by the zeckybot metallic mean analyzer: copper (n=3) contains gold at 7.28%, silver at 2.80%.

The σ₃ width ratios predict layer boundaries:
- Silver→Gold boundary: 0.214 R (matches solar nuclear core at 0.20–0.25 R☉, 7% accuracy)
- Gold→Bronze boundary: 0.508 R

### The Dirac Dispersion as Discriminant Triangle

The relativistic energy-momentum relation maps directly onto the Pythagorean triangle:

$$E² = p²c² + m²c⁴ \quad \longleftrightarrow \quad 13 = 5 + 8$$

| Physics | Discriminant | Layer |
|---|---|---|
| m²c⁴ (mass²) | Δ₂ = 8 (silver) | Innermost core, confinement |
| p²c² (momentum²) | Δ₁ = 5 (gold) | Middle shell, propagation |
| E² (observable²) | Δ₃ = 13 (bronze) | Surface, measurement |

A particle at rest (p = 0): E = mc² → pure silver (confinement only). A massless particle (m = 0): E = pc → pure gold (propagation only). A general particle: E = √(p²c² + m²c⁴) → bronze (surface) = Pythagorean combination.

### Physical Consequence

The bronze wave is not independent — its discriminant IS the Pythagorean combination of gold and silver. The third spatial dimension is **emergent** from the first two:

- **Silver** (√8): confinement/mass axis → innermost
- **Gold** (√5): propagation/momentum axis → middle
- **Bronze** (√13 = √(5+8)): observable/surface → emergent hypotenuse
- **n = 4** (√20 ≠ √21): chain breaks → no 4th dimension

The 3D vacuum Hamiltonian uses all three frequencies:

$$\alpha_1 = \sqrt{2}-1 \text{ (silver, mass)}, \quad \alpha_2 = \frac{\sqrt{5}-1}{2} \text{ (gold, momentum)}, \quad \alpha_3 = \frac{\sqrt{13}-3}{2} \text{ (bronze, observable)}$$

Each axis at the critical point V = 2J. The matter fraction: Ω_b ≈ W⁴ = (0.467)⁴ = 0.048, matching Planck (0.049) at 3.4%. Alternatively, Ω_b ≈ e⁻³ ≈ 0.050, where the exponent 3 counts the three metallic means whose discriminants form a Fibonacci chain.
...
- Discriminant Fibonacci chain 5+8=13: **exact arithmetic**, uniqueness proof (n-2)²=0
- Pythagorean triple (√5)² + (√8)² = (√13)²: **exact arithmetic**
- Concentric nesting (silver inner, gold middle, bronze outer): **computed from AAH band structure**, confirmed by zeckybot analyzer
- Solar core boundary at 0.214R from σ₃ ratios: **7% match** to observed 0.20–0.25 R☉
...
- γ\_dc = 4 from Chern number counting: **~80% confidence**. Supported by Liu et al. 2020 and Zhang et al. 2022. Not yet a closed-form proof.
...
- Dirac mapping 13 = 5 + 8 ↔ E² = p²c² + m²c⁴: **structural correspondence exact**, physical identification (mass = silver, momentum = gold) supported by five independent lines of evidence but not derived from 3D AAH effective theory.
...
- 3D AAH simulation to verify emergent Dirac fermions at the triple critical point
...
*Last Updated: March 16, 2026 | v2: Pythagorean triangle, corrected nesting, Dirac mapping*
