...
### Concentric Nesting (corrected March 16, 2026)
...
### Axis-Specific Mode Selector

The dark fraction per axis changes the mode selector θ:
- Silver axis: θ = 0.171 (17.1% visible, 82.9% dark)
- Gold axis: θ = 0.236 (23.6% visible, 76.4% dark)
- Bronze axis: θ = 0.394 (39.4% visible, 60.6% dark)

**W⁴ = 0.048 (matter fraction) is UNCHANGED** — W is universal for all irrational α at V=2J.
The axis-specific θ values affect the DM/baryon SPLIT, not the total matter fraction.

**Open problem:** The formula connecting axis-specific dark fractions to Ω_DM/Ω_b.
...
PHI = (1 + math.sqrt(5)) / 2
D1, D2, D3 = 5, 8, 13
assert D1 + D2 == D3                                    # Fibonacci chain
assert abs(math.sqrt(D1)**2 + math.sqrt(D2)**2 - math.sqrt(D3)**2) < 1e-12  # Pythagorean
cos_theta = math.sqrt(5) / math.sqrt(13)                # 0.6202 ≈ 1/φ
sigma3_silver, sigma3_gold, sigma3_bronze = 0.1712, 0.2361, 0.3941
sigma3_total = sigma3_silver + sigma3_gold + sigma3_bronze
r_silver_boundary = sigma3_silver / sigma3_total         # 0.214 R
r_gold_boundary = (sigma3_silver + sigma3_gold) / sigma3_total  # 0.508 R
```
...
Supported by: Liu, Fulga & Asbóth (2020), Varjas et al. (2025), 36 total references.
...
**Preprint submitted to Research Square, March 16, 2026.**
PDF (LaTeX, 16pp), DOCX, and MD versions with 5 figures and 11 computational proofs.
...
```
Schrödinger equation → AAH Hamiltonian → Hofstadter butterfly
    → metallic mean hierarchy → discriminants 5+8=13
    → Pythagorean = Dirac: E²=p²c²+m²c⁴
    → non-relativistic limit → Schrödinger
```
...
| 0.30 | 8.450 | 5.3% | Schrödinger breaking |
| 0.50 | 9.250 | 13.5% | Dirac needed |
| 1.00 | 13.000 | 38.5% | Fully relativistic (bronze) |

### Hierarchy of Theories

| Theory | Triangle position | Discriminant |
|---|---|---|
| Rest mass (E=mc²) | Silver vertex | Δ = 8 |
| Schrödinger (p²/2m) | Tangent at silver | Δ ≈ 8 + ε |
| Dirac (E²=p²c²+m²c⁴) | Full hypotenuse | 8 ≤ Δ ≤ 13 |
| QFT (fields) | Bronze surface | Δ = 13 |
...
| **11 — Concentric nesting** | **CONFIRMED** | **Silver inner (σ₃=0.171), Gold middle (0.236), Bronze outer (0.394)** |
| **12 — Dirac mapping** | **STRONG CONJECTURE** | **E²=p²c²+m²c⁴ ↔ 13=5+8; structural exact, not derived from 3D AAH** |
...
│   ├── Hofstadters_Golden_Butterfly.md ← Metallic mean hierarchy ★★ NEW
│   ├── Three_Dimensions_Proof.md     ← Pythagorean triangle v3 ★★ NEW
│   ├── Dirac_Mass_Fibonacci.md       ← E²=p²c²+m²c⁴ ↔ 13=5+8 ★★ NEW
│   ├── Schrodinger_Tangent_Line.md   ← NR limit = tangent ★★ NEW
│   ├── Confinement_Chern_Pair.md     ← 5→3 as Chern annihilation ★★ NEW
│   ├── Mode_Selector_Formula.md      ← Four-mode collapse ★★ NEW
...
│   ├── Hofstadter_Proof.py      ← 11/11 computational proofs ★★ NEW
│   ├── bronze_emergence.py      ← Pythagorean + nesting ★★ NEW
│   ├── pauli_reassignment.py    ← Silver=σ_z mapping ★★ NEW
...
│   ├── Three_Dimensions.md      ← v3 with Pythagorean + nesting ★★
│   └── Hofstadters_Golden_Butterfly.pdf ← Research Square preprint ★★ NEW
...
│   ├── Hofstadters_Golden_Butterfly.tex  ← LaTeX source ★★ NEW
│   └── Hofstadters_Golden_Butterfly.docx ← Word with figures ★★ NEW
...
*v7.0 — Updated March 16, 2026 (Hofstadter, Pythagorean, nesting correction, Dirac, Schrödinger)*
