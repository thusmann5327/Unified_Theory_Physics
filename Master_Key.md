...
5A. [Galaxy Rotation — Hofstadter Butterfly](#va-galaxy-rotation)
...
> **NOTE (March 20, 2026):** The backbone propagator parameters below are all exact theorems and remain valid. However, for galaxy rotation curves specifically, the backbone formula (35% RMS, Grok-verified FAIL) is **superseded** by the Hofstadter butterfly path (2.7% RMS). See §V-A below.
...
# V-A. GALAXY ROTATION — HOFSTADTER BUTTERFLY PATH (March 20, 2026)

## The Physical Picture

A spiral galaxy IS Harper's lattice in a magnetic field. The gravitational potential maps to the coupling ratio V/J, and the galactic magnetic field maps to the flux parameter α. As both vary with radius, the galaxy traces a 2D path through the Hofstadter butterfly.

The Anderson localization transition — from trapped matter (inner bulge, V/J > 2) through fractal criticality (scale radius, V/J = 2) to free matter (outer halo, V/J < 2) — produces a gate transmission that partially compensates the 1/r gravitational decline. The result is an approximately flat rotation curve.
...
# Magnetic field → flux parameter (ε = W⁴ DERIVED from spectrum)
...
## The ε = W⁴ Derivation (NOVEL)

The magnetic perturbation ε = W⁴ = 0.0476 is **derived** from the AAH spectrum, not fitted. It equals the cosmic baryon fraction Ω_b = 0.0486 to 2%. Physical basis: B-field generated by baryons → perturbation ∝ baryon fraction.

**Test:** ε = W⁴ (derived) gives **2.7% RMS** vs ε = 0.05 (fitted) gives 3.4% RMS. The derived value **outperforms** the fitted value.

## Zero-Parameter BTFR Chain

| Step | Formula | Source |
|------|---------|--------|
| 1 | a₀ = c²/(l_P × φ²⁹⁵) = 1.241×10⁻¹⁰ m/s² | AAH spectrum (3.4%) |
| 2 | v_flat = (G × a₀ × M_baryon)^(1/4) | Baryonic Tully-Fisher |
| 3 | M_ref from butterfly integral at v_flat | Hamiltonian |
| 4 | v(r) from butterfly path | Hamiltonian |

**Result:** NGC 3198 → v_BTFR = 149 km/s (observed: 148 km/s, **ERROR: 1%**). Zero fitted parameters.

## Missing Baryon Prediction (NOVEL — TESTABLE)

**M_CGM = v_flat⁴/(G×a₀) − M_visible**

No other dark matter model makes per-galaxy CGM mass predictions.

| Galaxy | M_CGM Predicted | Observed | Status |
|--------|----------------|----------|--------|
| NGC 3198 | ≈ 0 (baryon census complete) | — | Consistent |
| NGC 2403 | 1.1×10¹⁰ M☉ | — | **Testable** (X-ray) |
| NGC 6946 | 4.7×10¹⁰ M☉ | — | **Testable** (X-ray) |
| **Milky Way** | **8.2×10¹⁰ M☉** | **3-10×10¹⁰ M☉** | **IN RANGE** |
| UGC 2885 | 2.9×10¹¹ M☉ | — | **Testable** (X-ray) |

**Milky Way cross-check:** M_baryon_total = 1.42×10¹¹ M☉. Cosmic expectation (Ω_b/Ω_m × M_total): 1.38×10¹¹ M☉. Ratio: **1.03** — galaxy retains 103% of cosmic baryon allotment.

## Universal Rotation Curve Shape

When normalized to v/v_flat vs r/r_s, the butterfly produces a universal shape across **3 orders of magnitude in galaxy mass** (DDO 154 through UGC 2885). Matches Persic & Salucci (1996) universal rotation curve.
...
The exponent 4 comes from L = 1/φ⁴: ln(1/φ⁴)/ln(φ) = −4.

## Results Comparison

| Model | Free Params | RMS (10-30 kpc) | Status |
|-------|------------|-----------------|--------|
| Backbone (v1) | 0 | 35% | **SUPERSEDED** (Grok #2 FAIL) |
...
## Adversarial Verification (4 Rounds, Grok/xAI)

| Round | Key Result |
|-------|-----------|
| 1 | Koch q-desic bridge: **DEAD** |
| 2 | Atomic scorecard: **PASS**; Backbone rotation: **FAIL** (35%) |
| 3 | Opening gate: **TAUTOLOGY** |
| 4 | Butterfly robustness: **PASS**; V/J mapping: **PASS**; NGC 2403 transfer: **PASS** |

## Open Questions

1. **Gravity × gate ≈ constant** — emergent, not proven as theorem
2. **r_gate = 5.85 × r_s** — testable with SKA/MeerKAT
3. **α₀ = 1/φ derivation** — may connect to golden-angle phyllotaxis
4. **ε = W⁴ vs ε = Ω_b** — differ by 2%, below current precision

See: `theory/Hofstadter_Butterfly_Galaxy_Rotation.md`
...
| 22 | **Galaxy rotation (butterfly)** | **v(r) shape** | **Butterfly path + W⁴** | **2.7% RMS** |
| 23 | **NGC 3198 v_flat (BTFR)** | **149 km/s** | **(G×a₀×M_bar)^(1/4)** | **1%** |
| 24 | **MW CGM mass** | **8.2×10¹⁰ M☉** | **v⁴/(Ga₀)−M_vis** | **In range** |
| 25 | **Universal rotation curve** | **7 galaxies** | **Butterfly normalization** | **Persic-Salucci** |

**25 independent predictions. Three constants. Zero free parameters. All from φ² = φ + 1.**

**Plus 6 novel testable predictions** (per-galaxy CGM masses) that no other dark matter model makes.
...
### Galaxy Rotation (March 20, 2026)

The backbone propagator (§V) is superseded for rotation curves by the Hofstadter butterfly path (§V-A). The butterfly uses the same AAH Hamiltonian but traces a 2D path through parameter space rather than applying a single critical-point formula. Key advancement: ε = W⁴ is derived from the spectrum and outperforms fitted values. The BTFR zero-parameter chain gives NGC 3198 at 1% error. Six novel per-galaxy CGM mass predictions are testable with XRISM, Athena, and SKA.
...
[12] Persic, M. & Salucci, P. ApJS 99, 501 (1996).
[13] Husmann, T.A. "Hofstadter Butterfly Galaxy Rotation." theory/Hofstadter_Butterfly_Galaxy_Rotation.md (2026).
